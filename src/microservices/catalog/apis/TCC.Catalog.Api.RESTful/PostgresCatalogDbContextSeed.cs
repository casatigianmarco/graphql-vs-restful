...
using Polly;
using Polly.Retry;
using TCC.Base.Extensions;
using TCC.Catalog.Domain.Entities;
using TCC.Catalog.Persistence.Postgres.EntityFrameworkCore;

namespace TCC.Catalog.Api.RESTful;

public class PostgresCatalogDbContextSeed
{
    public async Task SeedAsync(PostgresCatalogDbContext context, IWebHostEnvironment env, IOptions<CatalogSettings> settings,
        ILogger<PostgresCatalogDbContextSeed> logger)
    {
        var policy = CreatePolicy(logger, nameof(PostgresCatalogDbContextSeed));
    ...
    private IEnumerable<CatalogItem> GetCatalogItemsFromFile(string contentRootPath, PostgresCatalogDbContext context,
        ILogger<PostgresCatalogDbContextSeed> logger)
    {
    ...
    private AsyncRetryPolicy CreatePolicy(ILogger<PostgresCatalogDbContextSeed> logger, string prefix, int retries = 3)
    {
